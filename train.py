...
def compute_advantage_and_value_targets(rewards, values, dones, gamma, lam):
    ...
    for t in reversed(range(len(rewards) - 1)):
        ...
        # ADV
        delta_t = (
            rewards[t] + (gamma * (values[t + 1]) * int(not dones[t + 1])) - values[t]
        )
...
def train(args):
    ...
    for actor in range(actors):
        ...
        env = Atari_Wrapper(
            raw_env, env_name, num_stacked_frames, use_add_done=args.lives
        )
    ...
    while runner.cur_step < total_steps:
        ...
        # get data
        batch_obs, batch_actions, batch_adv, batch_v_t, batch_old_action_prob = (
            None,
            None,
            None,
            None,
        ...
        for env_runner in env_runners:
            ...
            adv, v_t = compute_advantage_and_value_targets(
            ...
            batch_obs = (
                torch.stack(obs[:-1])
                if batch_obs == None
                else torch.cat([batch_obs, torch.stack(obs[:-1])])
            )
            batch_actions = (
                np.stack(actions[:-1])
                if batch_actions is None
                else np.concatenate([batch_actions, np.stack(actions[:-1])])
            )
            batch_adv = (
                torch.stack(adv)
                if batch_adv == None
                else torch.cat([batch_adv, torch.stack(adv)])
            )
            batch_v_t = (
                torch.stack(v_t)
                if batch_v_t == None
                else torch.cat([batch_v_t, torch.stack(v_t)])
            )
            batch_old_action_prob = (
                torch.stack(old_action_prob[:-1])
                if batch_old_action_prob == None
                else torch.cat(
                    [batch_old_action_prob, torch.stack(old_action_prob[:-1])]
                )
            )
        ...
        dataloader = DataLoader(
            dataset, batch_size=minibatch_size, num_workers=0, shuffle=True
        )
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(dataloader):
                ...
                # compute clip*A_t
                clip = (
                    torch.clamp(probability_ratio, 1 - current_eps, 1 + current_eps)
                    * adv
                )
        ...
        if runner.cur_step % 50000 < T * actors:
            end_time = time.time()
            print(
                f"*** total steps: {runner.cur_step} | time(50K): {end_time - start_time} ***"
            )
            start_time = time.time()
